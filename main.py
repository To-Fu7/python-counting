...
DEBUG_SENT_DATA = os.getenv('DEBUG_SENT_DATA', 'false').lower() == 'true'
...
def send_person_in_mqtt(cropped_image, record_id, event_type="person_in"):
    ...
    if DEBUG_MODE and not DEBUG_SENT_DATA:
...
def get_video_source():
    ...
    # Webcam device index (e.g. RTSP_URL=0, 1, 2)
    ...
    while True:
        try:
            ...
            while True:
                ...
                results = model.track(detection_frame, persist=True, verbose=False, conf=YOLO_CONFIDENCE, device=resolved_device)
